...
- **Budget domain utilities:** `frontend/src/utils/budgetUtils.js` is the single source of truth for budget calculation constants and helpers: `WARNING_THRESHOLD`, `getBudgetZone(actual, budgeted)` → `'safe'|'warning'|'over'|'no-budget'|'no-data'`, `getPillAriaLabel(actual, budgeted, zone)` → accessible string. All budget components (table, pill, etc.) import from here — do not redefine these in component files or in `chartUtils.jsx`.
