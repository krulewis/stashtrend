...
- **HorizontalSwipeContainer dot bar:** The 3-dot view indicator sits in a `position: fixed` strip (28px tall) with `background: var(--bg-root)` and `border-top: 1px solid var(--border)`, positioned directly above the BottomTabBar. The pane container height subtracts this 28px: `calc(100dvh - 60px - 28px - 56px - env(safe-area-inset-bottom))`. Content scrolls underneath the dot bar — it must never float transparently over pane content. Touch targets are 44×28px with 8px visual dots centered via `::before` pseudo-elements.
