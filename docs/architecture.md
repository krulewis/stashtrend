...
- **Backend:** Blueprint-modularized Flask app (Phase B complete, PR #18). Port 5050.
  ...
  - **`backend/routes/`** — 10 Flask Blueprints (one file each): `setup.py`, `settings.py`, `retirement.py`, `groups.py`, `budgets.py`, `networth.py`, `sync.py`, `ai_routes.py`, `budget_builder.py`, `investments.py`. Route modules use `import app as _app` to preserve all `patch("app.X")` test mocks.
...
- **Sidebar Navigation + URL Routing:** COMPLETE — react-router-dom v6, `<Sidebar>` (desktop) + `<BottomTabBar>` (mobile), `nav.js` single source of truth, `NetWorthPage` extracted from App, `AppShell` inner component for `useLocation` focus management, routes: `/networth`, `/groups`, `/budgets`, `/builder`, `/sync`, `/milestones` (was `/forecasting`, redirect preserved) with `/` and `*` redirects to `/networth` ✅
...
- **NetWorthChart removed (2026-03-12):** The standalone total-NW-over-time line chart (`NetWorthChart.jsx`) was removed from `NetWorthPage`. The page now fetches 3 endpoints (stats, accounts, by-type) instead of 5. `fetchNetworthHistory` is no longer called from `NetWorthPage`. The `TypeStackedChart` (stacked area by bucket type) remains.
- **Milestones Page Consolidation (2026-03-12):** Renamed `ForecastingPage` route from `/forecasting` to `/milestones` (HTTP redirect preserved). Moved `MilestoneCardsView` + `RetirementPanel` from `NetWorthPage` into `ForecastingPage` as new sections. Deleted `MilestoneSkylineView.jsx`, `MilestoneSkylineView.module.css`, `MilestoneSkylineView.test.jsx`, `MilestoneHeroCard.jsx`, `MilestoneHeroCard.module.css`, `MilestoneHeroCard.test.jsx`. `NetWorthPage` now renders: StatsCards → TypeStackedChart → AccountsBreakdown (3 API fetches: `GET /api/stats`, `GET /api/accounts/summary`, `GET /api/networth/by-type`). Retirement data removed from `NetWorthPage` fetch. `ForecastingPage` now renders: `ForecastingSummary` → `MilestoneCardsView` → `ForecastingChart` → `ForecastingControls` → `RetirementPanel`. `useMilestoneData` hook receives `(typeData, retirement)` props (no new API calls). Page h1="Milestones". Bug fixed: `isRetirementTargetInvalid` was checking `years != null && years <= 0` but `years` useMemo returns null when y≤0; now computes raw y directly.
