...
A list (and pie chart) of all your synced accounts with their current balances, so you can see at a glance where your money lives.
...
Below the stacked chart, StashTrend calculates the **Compound Annual Growth Rate** for each bucket over 1, 3, and 5 year windows, so you can see which parts of your portfolio are growing fastest.
...
- StashTrend detects if the same account is in conflicting groups and warns you before saving
...
- **Net** — income minus expenses for each month
...
Based on your profile and regional data, the AI produces a suggested monthly budget for every Monarch category. The results appear in an editable table — adjust any line before applying.
...
You can save any budget plan with a name and timestamp and reload it later. Multiple plans can be stored, making it easy to compare a conservative vs. aggressive spending plan, for example.
...
A table of past sync jobs showing:
- Start and end time
- Duration
- Entities synced
...
AI features are optional and require you to supply your own API key. StashTrend supports three provider types:

| Provider | Models |
|---|---|
| **Anthropic** | Claude models (claude-opus-4-5, claude-sonnet-4-5, etc.) |
| **OpenAI** | GPT models (gpt-4o, gpt-4o-mini, etc.) |
| **OpenAI-compatible** | Ollama, Groq, or any provider with a custom base URL |

### Configuring AI

Navigate to **Settings** (accessible from the Budget Analysis panel or Budget Builder) to enter your provider, model, and API key. Keys are stored in your system keychain (with a secure file fallback in Docker environments).
...
Retirement planning is embedded in the **Net Worth Dashboard** as a collapsible panel.
...
Add custom financial milestones — for example, "Pay off mortgage" at $0 debt, or "Reach $1M" — with target dollar amounts. Milestones appear as labeled reference lines on the Net Worth Over Time chart.

### Retirement Summary

A summary panel shows:
- **Projected nest egg** at your target retirement age
- **Required nest egg** based on your desired income and the 4% safe withdrawal rule
- **On-track status** — a color-coded badge indicating whether your projected savings meets your goal
- Social Security offset applied to the required nest egg calculation
...
Holdings data is the foundation for future per-account performance analysis features.
...
- **Local-only CORS policy** — the API only accepts requests from `localhost` and `127.0.0.1`; no external origin can query it
- **No external data transmission** — your financial data never leaves your machine (AI features send only the data you explicitly submit to the analysis)
