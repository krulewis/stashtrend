# Phases 3-4 Integration — Final Implementation Plan

**Date:** 2026-03-12
**Author:** Engineer Agent (Final Plan — PP-6)
...
The automated staff reviewer (PP-5) was unable to complete its review due to a usage limit error. In lieu of a full staff review pass, the orchestrator performed a manual verification of the initial plan against the actual codebase and produced three findings. This final plan incorporates all three findings as a delta patch against the initial plan. No findings were Critical or High severity — the initial plan required only one low-severity clarification and two confirmations.

### Review Finding Responses

**Finding 1 (Low): `retirement.py` uses `current_app` import — inconsistency note**

The reviewer noted that `retirement.py` (line 3) imports `current_app` from Flask, which is inconsistent with the plan's decision to use a module-level `logger` in `investments.py`. This is documented and the plan position is unchanged: `investments.py` will use `logger = logging.getLogger(__name__)` (module-level, context-free) rather than `current_app.logger`. The inconsistency with `retirement.py` is pre-existing and is not a blocker. The architecture decision correctly notes that the module-level logger is architecturally superior (works without a pushed app context, no import dependency on Flask's application proxy). The plan adds a comment in Change A1 directing the implementer not to follow `retirement.py`'s pattern on this point.

**Finding 2: All plan assumptions verified correct**

All factual assertions in the initial plan were verified against the actual codebase:
- `investments.py` does NOT exist (confirmed — must be created)
- Frontend components all exist in `frontend/src/components/` and `frontend/src/pages/` flat directories (not subdirectories)
- `routes/__init__.py` has 9 blueprints, investments not among them (confirmed)
- `test_helpers.py` `make_test_db()` applies both PIPELINE_DDL and DASHBOARD_DDL (confirmed)
- `retirementMath.test.js` exists with Phase 2 tests only (confirmed — new `describe` blocks can be appended)
- Frontend test files use Vitest (`describe`, `it`, `expect` from 'vitest') (confirmed)
- `api.js` exports `fetchInvestmentsSummary`, `fetchInvestmentsHoldings`, `fetchInvestmentsPerformance` (confirmed)

No plan changes required by this finding — it is a confirmation pass.

**Finding 3: No critical or high severity issues**

The plan is comprehensive and well-structured. The four parallel streams are genuinely independent. No additional changes required.
...
6. **Logger pattern diverges from `retirement.py` (new — from review Finding 1):** `retirement.py` uses `current_app` (imported from Flask) for its logger. `investments.py` will NOT follow this pattern. The module-level `logger = logging.getLogger(__name__)` is the correct approach: context-free, works in tests without a pushed Flask app context, and is what Flask documentation recommends for Blueprint modules. The implementer must not copy `retirement.py`'s logger pattern — use the module-level logger defined in Change A1.
...
**IMPORTANT — Logger pattern:** Do NOT follow `retirement.py`'s use of `current_app`. Use module-level `logger = logging.getLogger(__name__)` throughout. `retirement.py` imports `current_app` from Flask (line 3), but that is an older pattern. The module-level logger is context-free and correct for this module.
...
```
Note: Nullable fields (`total_cost_basis`, `total_return_dollars`, `total_return_pct`, `cagr_pct`) will be `null` when data is unavailable — never omitted.
...
```
Returns 404 JSON `{"error": "Account not found"}` if account_id is not found or is not an investment account.
...
3. **Create** `backend/tests/test_investments.py` — 18 test cases across 3 endpoint groups
4. **Create** 10 frontend test files for Phase 3-4 components
5. **Extend** `retirementMath.test.js` with 3 new `describe` blocks (19 test cases)
