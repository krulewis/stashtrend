...
- **Budget Heatmap View** — Mobile-only 6-month heatmap as pane 0 in `HorizontalSwipeContainer`. Plan: `heatmap-impl-plan.md`. Architecture: `heatmap-architecture.md`. Design: `heatmap-design-spec.md`. Groups A–F all complete. **Group A (Shared Utilities): complete** — `groupExpenses()` and `formatMonthLabel()` in `budgetUtils.js` (18 new tests). **Group B (MonthDetailView Refactor): complete** — split useMemo into grouping + month extraction. **Group C (HorizontalSwipeContainer): complete** — configurable `labels` prop (3 new tests). **Group D (New Components): complete** — `WindowPicker.jsx` + `HeatmapView.jsx` + CSS modules. **Group E (Page Integration): complete** — `MobileBudgetPage.jsx` updated for 3-pane layout. **Group F (Tests): complete** — `WindowPicker.test.jsx` (10 tests) + `HeatmapView.test.jsx` (13 tests). All 607 frontend tests passing. **Status: pending QA, commit, PR, review loop.**
