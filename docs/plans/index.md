...
- **Milestones Page Consolidation** (M): COMPLETE — Renamed `/forecasting` → `/milestones` (redirect preserved). MilestoneCardsView + RetirementPanel consolidated into ForecastingPage. MilestoneSkylineView + MilestoneHeroCard deleted. NetWorthPage now 3-fetch lean version. Bug fixed: isRetirementTargetInvalid validation. Committed 2026-03-12, PR pending push.
